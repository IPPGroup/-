...
<%  
    ...
    rs2=stmt.executeUpdate("UPDATE hotel.myorder SET status='已取消' WHERE (pnumber='"+cancel+"')");
   	rs=stmt.executeQuery("select * from myorder where status='已取消'");
   	ArrayList<String> hotelname = new ArrayList<String>();
	...
	while(rs.next()){
		...
		num.add(rs.getInt("roomNum"));
	}
	for(int i=0;i<hotelname.size();i++){
		String name=hotelname.get(i);
		String type=roomtype.get(i);
		int numb=num.get(i);
		Statement stm = conn.createStatement();
		stm.executeUpdate("update roominfo set number=number+'"+numb+"'where hotelname='"+name+"'and roomstyle='"+type+"'");
	}
    request.getRequestDispatcher("myOrder1.jsp").forward(request, response);
	}
...
%>  
	...
	<script type="text/javascript">
		if(document.referrer=="http://localhost:8080/Hotelroom/%E8%AE%A2%E9%85%92%E5%BA%972"){
			$("#back").click(function() {
